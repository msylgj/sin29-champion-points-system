...
def get_configuration(config_id: int, db: Session = Depends(get_db), _auth: dict = Depends(verify_admin_token)):
    """获取单个赛事配置的详细信息"""
...
def get_event_configurations(event_id: int, db: Session = Depends(get_db), _auth: dict = Depends(verify_admin_token)):
    """获取某赛事的所有配置"""
